...
public class RightS extends Shape{
	...
	private static int LocalShapeColor = 4;
	protected final Engine.ShapeType type = Engine.ShapeType.S;
	private enum RotatePositions {
		UP, DOWN, LEFT, RIGHT
	}
	public RightS(Space[][] gameBoard) {
		super(gameBoard);
		Space shapeSpaces[] = new Space[] {gameBoard[0 + this.verticalOffset][this.startColumnLeftMost],gameBoard[1 + this.verticalOffset][this.startColumnLeftMost],gameBoard[1 + this.verticalOffset][startColumnRightMost],gameBoard[2 + this.verticalOffset][startColumnRightMost]};
	...
	public RightS getInstance(Space[][] gameBoard) {
		return new RightS (gameBoard);
	}
	...
	protected boolean canRotate() {	
		try {
			...
			case UP : {
				...
				if (this.gameBoard[coordsOfSpace_0[0]][coordsOfSpace_0[1]+1].getColor() == 0 && this.gameBoard[coordsOfSpace_0[0]][coordsOfSpace_0[1]+2].getColor() == 0) {
					return true;
				}
				else return false;
			}
			case RIGHT : {
				...
				int[] coordsOfSpace_0 = this.getCoordsOfSpace(this.spaces[0]);
				if (this.gameBoard[coordsOfSpace_0[0]+1][coordsOfSpace_0[1]].getColor() == 0 && this.gameBoard[coordsOfSpace_0[0]+2][coordsOfSpace_0[1]].getColor() == 0) {
					return true;
				}
				else {
					return false;
